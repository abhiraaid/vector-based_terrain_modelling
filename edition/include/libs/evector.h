...
public:
  ...
  friend QString ToGLSL(const Vector&);
...
/*!
\brief Raise the components to a power.
\param e Exponent.
*/

inline Vector Vector::Pow(const double& e) const
{
  return Vector(Math::Pow(c[0], e), Math::Pow(c[1], e), Math::Pow(c[2], e));
}
...
/*!
\brief Compute the index of the minimum component of a vector.
...
\sa MaxIndex
*/
inline int Vector::MinIndex() const
{
  if (c[0] <= c[1])
  {
    if (c[0] <= c[2])
...
public:
  ...
  friend QString ToGLSL(const Vector2&);
...
\sa Vector::MinIndex
*/
inline int Vector2::MinIndex() const
{
  if (c[0] <= c[1])
  {
    return 0;
...
\param p Point
\param a, b Vertexes of the line.
*/
