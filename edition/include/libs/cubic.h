...
public:
  ...
  friend Cubic operator/ (const Cubic&, const double&);
  friend Cubic operator* (const Quadric&, const Linear&);
  ...
  friend std::ostream& operator<<(std::ostream&, const Cubic&);
...
inline Cubic::Cubic(const Quadric& p)
{
  c[3] = 0.0;
  c[2] = p[2];
  c[1] = p[1];
...
/*!
\brief Multiply a quadric by a linear polynomial.
\param q %Quadric.
\param l %Linear.
*/
inline Cubic operator* (const Quadric& q, const Linear& l)
{
  return Cubic(q[2] * l[1], q[2] * l[0] + q[1] * l[1], q[1] * l[0] + q[0] * l[1], q[0] * l[0]);
}
...
\sa Cubic::SmoothCompact(const double&, const double&)

\param x Distance (not the squared distance).
\param ra, rb Radius interval.
*/
inline double Cubic::SmoothCompact(const double& x, const double& ra, const double& rb)
{
  if (x < ra) return 1.0;
  else if (x > rb) return 0.0;
  else
  {
    double y = (x - ra) / (rb - ra);
    return Math::Cube(1.0 - y * y);
  }
}
...
*/
inline double Cubic::Warp(const double& x)
{
  const double y = 2.0 * x;
  if (x < 0.5)
  {
    return 0.5 * Math::Cube(y);
  }
  else
  {
    return 1.0 - 0.5 * Math::Cube(2.0 - y);
  }
}
